...
// .popover         - Wrapper
// .popover-a       - <button>
// .popover-b       - data-popover-type="popover" | data-popover-type="dialog" | Responsiveness at small screen sizes (height) - height: calc(100vh - 4rem); The 4rem will be changed to be that of the headers height
// .popover-b-close - <button> with aria-label="Close menu"
// .popover-b-overlay - <div id="#" class="popover_b_overlay"></div> | popover_b needs .has_overlay + data-overlay-target="#ID" ← (HASHTAG SYMBOL NEEDED)
// .popover-a.popoverActiveColor - Sets the active styling for popover_a
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const showOverlay = (selector) => {
    document.querySelectorAll('.popover-b-overlay').forEach(o => (o.style.display = 'none'))
    if (selector) {
  ...
  const isNestedPopover = (popover) => {
    return popover.closest('.popover-b') && popover.closest('.popover-b') !== popover
  }
  ...
  const closeAllPopovers = () => {
    ...
    while (openPopovers.length > 0) {
      ...
      const pb = popover.querySelector('.popover-b')
      const pa = popover.querySelector('.popover-a')
      if (pb) {
  ...
  function getDirectChildCloseButton(pb) {
    ...
    for (let i = 0; i < children.length; i++) {
      if (children[i].classList.contains('popover-b-close')) return children[i]
    }
    return null
  }

  function activateNestedOpenPopovers(container) {
    const subPopovers = container.querySelectorAll('.popover-b')
    subPopovers.forEach(childPb => {
      if (window.getComputedStyle(childPb).display !== 'none') {
        const childPbClasses = [...childPb.classList].filter(c => c !== 'popover-b')
        childPbClasses.forEach(cls => {
          const matchingTrigger = container.querySelector(`.popover-a.${cls}`)
          if (matchingTrigger) {
  ...
  popovers.forEach((p, index) => {
    const pa = p.querySelector('.popover-a')
    const pb = p.querySelector('.popover-b')
    if (!pb || !pa) return
    ...
    if (window.getComputedStyle(pb).display !== 'none') {
      ...
      const overlayNeeded = pb.classList.contains('has-overlay')
    ...
    pa.addEventListener('click', (e) => {
      e.stopPropagation()
      const isRoot = !pb.parentElement.closest('.popover-b')
    ...
    pb.addEventListener('click', (e) => {
      e.stopPropagation()
      const clickedTrigger = e.target.closest('.popover-a')
      if (clickedTrigger) {
        let subClasses = [...clickedTrigger.classList].filter(cls => cls !== 'popover-a')
        if (subClasses.length > 0) {
          const allTriggers = pb.querySelectorAll('.popover-a')
          allTriggers.forEach(t => {
            t.classList.remove('popoverActiveColor')
          })
          clickedTrigger.classList.add('popoverActiveColor')
          const allPopovers = pb.querySelectorAll('.popover-b')
          ...
          for (let i = 0; i < subClasses.length; i++) {
            ...
            let found = pb.querySelector('.popover-b.' + checkClass)
            if (found) {
              match = found
              break
            }
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.popover')) {
      closeAllPopovers()
    } else if (e.target.classList.contains('popover-b-overlay')) {
      closeAllPopovers()
    }
  })
